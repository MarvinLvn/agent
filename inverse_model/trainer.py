...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(1, self.max_epochs + 1):
            ...
            early_stopping(validation_metrics.metrics["mse_art"], self.nn)
    ...
    def epoch_train(self, dataloader):
        ...
        for batch in tqdm(dataloader, total=nb_batch, leave=False):
            ...
            art_seqs_pred = self.nn(sound_seqs)
            reconstruction_loss = self.loss_fn(art_seqs_pred, art_seqs, seqs_mask)
            reconstruction_loss.backward()
            self.optimizer.step()
            epoch_record.add("mse_art", reconstruction_loss.item())
        return epoch_record
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, total=nb_batch, leave=False):
                ...
                reconstruction_loss = self.loss_fn(
                    art_seqs_pred, art_seqs, seqs_mask
                )
